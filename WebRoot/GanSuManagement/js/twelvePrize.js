...
$(function() {
	...
	$(document).ready(function(){
		 ...
		 * 异步加载奖品管理列表
		 */
		$.post("../twelvePrize/list.do",{},function(res){
			res = eval("("+res+")");
			var html = "";
	...
	$(document).on("click",".updatePrize",function(){
		...
		if(overplus == ""){
			...
			return false;
		}else if((overplus-amount)>0){
			alert("奖品剩余数量不能超过总奖品数");
			$("input[name=overplus]").focus();
			return false;
		}
		
		$.ajax({
			url:"../twelvePrize/update.do",
			type:"post",
			data:{
				"id":id,
				"name":name,
	...
	$(document).on("click","a[data-field=modifyPrize]",function(){
		...
		$.post("../twelvePrize/getid.do",{"id":id},function(res){
			res = eval("("+res+")");
	...
	function deletePrize(id){
		if(confirm("您确定删除该奖项吗？")){
			$.post("../twelvePrize/del.do",{"id":id},function(res){
				if(res == "true"){
					alert("奖项删除成功！");
					window.location.reload();
				}
