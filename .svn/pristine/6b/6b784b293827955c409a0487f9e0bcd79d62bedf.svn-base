...
function index_conts(){
	...
	(function(e) { //自执行函数实时把i传入e
		abc[e].onclick = function() { //  通过传入的e获取到哪个按钮发生了点击事件
			...
			setTimeout(function() {
				mui.openWindow({
					url: 'stage/discountgo/news_details.html?id='+ArticleId,
					id: 'news',
	...
	for(var i = 0; i < abc_diapu.length; i++) {
		(function(e) { //自执行函数实时把i传入e
			abc_diapu[e].onclick = function() { //  通过传入的e获取到哪个按钮发生了点击事件
				...
				setTimeout(function() {
					mui.openWindow({
						url: 'stage/discountgo/shop_details.html?id='+shop_details_Id_conts,
						id: 'shop_details',
						showType: 'zoom-fade-out',
						showTime: 200
					});
				 ...
				 if(getseckillByGoodsId_.label ==1){//秒杀
					// alert("秒杀")
					 ...
					 var endtime=getseckillByGoodsId_.end_time;
					    ...
					    miaosha_ ="<img src='stage/img/smallImg/a4.png' />";
					 ...
					 if(getTuangouByGoodsId_ !=null){//团购价格
							if(getTuangouByGoodsId_.label ==2){
								//alert("团购")
								miaosha_ ="<img src='stage/img/smallImg/a3.png' />";
								...
								part = 0;
								 ...
								 if(getLongpayhuodongByGoodsId_ != null){//龙支付
									 if(getLongpayhuodongByGoodsId_.label ==3){
										 //alert("龙支付")
											miaosha_ ="<img src='stage/img/smallImg/a2.png' />";
...
//			            alert(e) //弹出e（此处e为i实时对应值）
                        ...
                        var part = $(this).attr("part");
                        	...
                        	url_ ='stage/discountgo/details.html?id='+shop_details_Id+'&part='+part
	...
	var indexBannerImgStatus_= getAdvertByStatus("one_lb");
	//console.log("轮播图=="+indexBannerImgStatus_);
	indexBannerImgStatus_ = JSON.parse(indexBannerImgStatus_);
	//console.log(indexBannerImgStatus_);
	var indexBannerImg ="";
	for(var i = 0 ; i < indexBannerImgStatus_.length; i++){
		indexBannerImg +="<div  onclick=\"javascript:window.location.href='"+indexBannerImgStatus_[i].ad_url+"'\" class='swiper-slide' style='background-image:url("+indexBannerImgStatus_[i].ad_path+")'></div>";//"<div class='swiper-slide' style=\"background-image:url("+indexBannerImgStatus_[i].ad_path+")\"></div>;
	}
	...
	if(advertisementBoxListImgStatus_4 !=null){
