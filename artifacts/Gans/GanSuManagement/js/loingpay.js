...
$(function(){
	...
	/*添加或更新龙支付活动商品*/
	$(".addGroupGoods").click(function(){
		var url = "../adminlongpay/add.do";
		var msg = "添加龙支付商品成功！";
		addOrUpdate(url,msg);
		return false;
	});
...
function deleteTuangou(id){
	if(confirm("确认删除该数据？")){
		var result = ajaxSubmit("../adminlongpay/del.do",{id:id},false);
		if(result == true){
			alert("龙支付商品移除成功！");
...
function returnFormData(id){
	...
	$(".seckillAddBox").hide();
	$(".seckillWin").toggle();
	var result = ajaxSubmit("../adminlongpay/getid.do",{id:id});
...
function getTabelData(curpage){
	...
	var async = false;
	var result = ajaxSubmit("../adminlongpay/listpage.do",data,false);
	var html = "";
	var i = 0;
	$(result).each(function(index,object){
		$(object.list).each(function(index,obj){
			...
			if(goods!=null){
				...
				html += "<tr><td>"+i+"</td><td>"+goods.goods_name+"</td><td>"+shop.shop_name+"</td><td>"+obj.start_time+"</td><td>"+obj.end_time+"</td><td><span class='costList'>原价：<span style='color:red;'>"+goods.price+"</span>元</span><span class='costList'>活动价格："+obj.real_price+"元</span></td><td><span class='operationButBox' data-id='"+obj.id+"'><a href='javascript:;' data-field='see' class='butBox'>查看</a><a href='javascript:;' data-field='edit' class='butBox ml10'>编辑</a><a href='javascript:;' data-field='delete' class='butBox mt10'>移除</a></span></td></tr>";
			}
		});
	});
